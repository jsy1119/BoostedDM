# File: int_flux_plot.py
#
# Plotting the interstellar pion and dark matter flux including three body decays
# Should be run as python three_body_plot.py [location=kcl or home] [particle=pion or dm]
...
if __name__ == '__main__':
    ...
    if location == 'kcl':
        ...
        save_directory = '/mnt/james/allMyStuff/BoostedDM/data/pion'
    elif location == 'home':
        save_directory = '/Users/james/allMyStuff/BoostedDM/data/pion'
    ...
    if particle == 'dm':
        Ncuts = 5
        logcuts = np.linspace(-3.0, 0.0, Ncuts)
    ...
    if particle == 'pion':
        ...
        plt.text(np.power(10.0, 0.0), np.power(10.0, -5.1), r'$n_{\textrm{\small MC}} = 500, \, E_p^{\textrm{\small max}} \sim 1\,\textrm{TeV}$', fontsize=10, ha="center", va="center", bbox=dict(boxstyle="round", lw=0.5, ec=(0.0, 0.0, 0.0), fc=(1.0, 1.0, 1.0)))
        ...
        plt.title('Interstellar Pion Flux', fontsize=20)
        plt.bar(binmidpointsarr, heights, width=binwidths, bottom=np.power(10.0, -14), color='w', alpha=0.6, edgecolor=plt_color, linewidth=0.3, log=True, label='Pion Flux: $N_{\\textrm{\\tiny bins}} = $' + ' {}'.format(Nbins))
        ...
        axes = plt.axis()
        plt.axis([np.power(10.0, -0.5), np.power(10.0, 2.0), np.power(10.0, -8.0), np.power(10.0, -5.0)])

        plt.gca().set_xscale("log")
        plt.gca().set_yscale("log")
        #plt.savefig('plots/int_pion_flux.pdf', bbox='tight')
	plt.show()
    ...
    if particle == 'dm':
        plt.figure(figsize=(7,7))
        plt.text(np.power(10.0, -1.8), np.power(10.0, -5.8), r'$\textrm{BR}(\pi^0 \rightarrow \gamma\chi\chi) = 10^{-6}, m_V = 0.01 \, \textrm{GeV}, m_\chi = 0.001 \, \textrm{GeV}$', fontsize=8, ha="center", va="center", bbox=dict(boxstyle="round", lw=0.5, ec=(0.0, 0.0, 0.0), fc=(1.0, 1.0, 1.0)))
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        ...
        plt.title('Interstellar Dark Matter Flux', fontsize=20)
        if location == 'kcl':
            plot_pospelov()
        elif location == 'home':
            plot_pospelov(save_directory='/Users/james/allMyStuff/BoostedDM/data/')
        plt.bar(binmidpointsarr, heights, width=binwidths, bottom=np.power(10.0, -14), color='w', alpha=0.6, edgecolor=plt_color, linewidth=0.3, log=True, label='DM Flux: $N_{\\textrm{\\tiny bins}} = $' + ' {}'.format(Nbins))
        plt.plot(binmidpointsarr, heights, color=line_color, linestyle='-', linewidth=1.0)
        plt.plot(kernel_evals, kernel_pdf, c=kde_color, linestyle='-', lw=1.0, label='DM Flux: KDE Fit')
        np.save('miguel/int_energies.npy', kernel_evals)
        np.save('miguel/int_flux.npy', kernel_pdf)
        plt.legend(frameon=False, fontsize=8)
        axes = plt.axis()
        plt.axis([np.power(10.0, -3.0), np.power(10.0, 1.0), np.power(10.0, -14), np.power(10.0, -5.5)])

        plt.gca().set_xscale("log")
        plt.savefig('plots/int_dm_flux_pospelov.pdf', bbox='tight')
